...
scServices.dataSync = scOnLoads[scOnLoads.length] = scOnUnloads[scOnUnloads.length] = {
	_commitInterval: 60000,
	commit: function(){
		var vIsDirty = false;
		if(scServices.suspendDataStorage.isDirty()) {
			scServices.suspendDataStorage.commit();
			vIsDirty = true;
		}
		if(scServices.completionStorage.isDirty()) {
			scServices.completionStorage.commit();
			vIsDirty = true;
		}
		if(scServices.scorm2k4 && scServices.scorm2k4.isScorm2k4Active()) {
			if(scServices.assmntMgr.isDirty()) {
		...
		if(vIsDirty){
			...
			scServices.suspendDataStorage._Uncommited = false;
			scServices.completionStorage._Uncommited = false;
			scServices.assmntMgr._Uncommited = false;
		}
	},
	hasBackend: function(){
		if(scServices.scorm2k4 && scServices.scorm2k4.isScorm2k4Active()) {
	...
	onLoad: function(){
		if(this.hasBackend() && this._commitInterval>0) this._ThreadCommit = window.setInterval("scServices.dataSync.commit();", this._commitInterval);
	},
	setCommitInterval: function(pInterval){
		this._commitInterval = pInterval;
		if(this._ThreadCommit) {
			try{window.clearInterval(this._ThreadCommit);}catch(e){};
			this.onLoad();
		}
	},
	loadSortKey: "2dataSync",
	onUnload: function(){
		try{window.clearInterval(this._ThreadCommit);}catch(e){};
	},
	unloadSortKey: "4dataSync"
}
