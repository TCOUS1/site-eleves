...
window.outMgr = {
						...
						for (let i = 0; i < pItem.children.length; i++) {
							...
							if (pCounter !== null){
								if (!outMgr.fCounterExcludes[pItem.children[i].source] && vDepth <= outMgr.fCounterMaxDepth){
									counter++;
									pItem.children[i].counter = outMgr.xConvertCounter(counter, vDepth-1);
								}
							}
	...
	addCounters: function (pCounterFormat) {
		...
		const iMenuWalker = function (pUl, pPrefix) {
			...
			let counter = 0;for (let i = 0; i < vLis.length; i++) {
				...
				if (pPrefix !== null){
					if (!outMgr.fCounterExcludes[vSource] && vDepth <= outMgr.fCounterMaxDepth){
						counter++;
						vBd.elt("span", "counter", vLbl.firstChild).text(pPrefix + outMgr.xConvertCounter(counter, vDepth-1) + ". ");
					}
				}
				if (scPaLib.checkNode(".type_b", vLis[i])) iMenuWalker(scPaLib.findNode("chi:ul", vLis[i]), pPrefix === null && outMgr.fCounterExcludes[vSource] || vDepth > outMgr.fCounterMaxDepth ? null : pPrefix + outMgr.xConvertCounter(counter, vDepth) + ".");
			}
		}
